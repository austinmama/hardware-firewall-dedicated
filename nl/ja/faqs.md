...
いいえ。ファイアウォール・サービス (標準でも専用でも) とネットワーク・ゲートウェイ・デバイスを同じ VLAN に割り当てることはできません。
...
ハードウェア・ファイアウォール、ハードウェア・ファイアウォール (専用)、および FortiGate Security Appliance には、帯域幅に対する課金はありません。  さらに、これらの製品を利用すると、サーバーが応答しなければならないトラフィックを制限することで、全体的な帯域幅使用率を低減できます。
