...
ご使用のサーバーで {{site.keyword.data.cloud_notm}}インフラストラクチャーによって提供された **Red Hat Enterprise Linux (RHEL)** ライセンスを利用している場合は、追加で以下のとおり、サービス・ネットワークへのアクセスを許可する必要があります。これを行わないと、更新とライセンス交付が正しく機能しません。
