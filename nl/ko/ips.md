...
서버가 {{site.keyword.data.cloud_notm}} 인프라에서 제공한 **Red Hat Enterprise Linux(RHEL)** 라이센스를 사용하는 경우 다음과 같이 서비스 네트워크에 대한 액세스를 추가적으로 허용해야 합니다. 그렇지 않으면 업데이트 및 라이센싱이 제대로 작동하지 않습니다. 
